...
server, build, package launcher, automatic port handshake and scaffold.
...
`ensure`, `start`, and `restart` may remain the foreground owner of the exact
package child. Preserve the returned terminal session. The launcher requests
port `0`; the operating system allocates the port and the package publishes its
origin through the runtime protocol. Never choose, reserve, scan, document, or
kill by port.
...
"$SKILL_ROOT/scripts/storybook.sh" browser open @ui/storybook
...
scheduling only through readiness/render barriers, never focuses an OS window,
and restores emulation during cleanup.
