...
const contracts = Object.freeze({
  ...
  workbench: contract(
    ...
    "Авторский TSX использует обычный document со стандартными DOM-типами: Template привязывает его к Document компонента при сборке и сохраняет ссылку в callbacks. Native document страницы не заменяется. Пространственные компоненты используют общий контракт Space: position и target целиком, rotation в градусах XYZ, CSS-размеры и преобразования, атрибут dpi у базового DisplayElement. Центральная область HUD задаёт только layout bounds. Её отдельная рамка Preview отсутствует: border, border-radius и overflow принадлежат самому display. Display имеет постоянные размеры 600 × 337,5 мм, dpi=96 и scale: 1. Shell вписывает его целиком дистанцией и параллельным сдвигом ViewPoint с целью, сохраняя пропорции и поля. Размеры Display, его viewport, матрица 2268 × 1276 и раскладка содержимого не меняются при resize окна или HUD. README и story остаются прямым содержимым Display без декоративной обёртки. " +
