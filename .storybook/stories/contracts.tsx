...
const contracts = Object.freeze({
  ...
  app: contract(
    ...
    "Browser владеет Document, Canvas, циклом кадров и вводом страницы. App монтируется в semantic body; Workbench получает Space по принадлежности своего Display, а documentElement остаётся html. Диагностика того же Root предоставляет exact `@zavx0z/space` `XRSpaceElement` и `XRViewPointElement`. Projection допускает только `display | hud | space`: display использует настоящий `DisplayElement`, hud — `XRHUDElement`, а space получает `context.space` и `mountSpacePreview`. Child Root, Document, Canvas, Space или ViewPoint не создаются.",
  ...
  scaffold: contract(
    ...
    "storybook init reads the required non-empty package.json#label and creates .storybook data without a manifest label or redundant root README reference, plus optional runtime/story owner directories. Projects with workspaces need no --declaration list; legacy project/workspace compositions use explicit --declaration paths.",
