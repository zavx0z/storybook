...
const contracts = Object.freeze({
  routeTree: contract(
    ...
    "Versioned standalone package, project and optional workspace declarations resolve atomically into one immutable graph. App адресует содержимое страницы; MCP на этом этапе адресует только пакет webxr/nodes/node. Правило принадлежит @mcp/address, список пакетов берётся из того же canonical graph. Параметры и внутренние директории в MCP не допускаются. Пакет @storybook/route динамически разрешает текущую ветку по директориям и workspace-границам; отдельная карта адресов не создаётся. Exact package identities сохраняются, прежние применённые URL действуют до проверенного применения новой ревизии. Package, category and subject overviews remain real states; unknown routes fail closed. Category and subject overviews render real immediate child stories without selecting their representative routes. A single-child overview retains that representative subject's Inspector contract and values.",
    ...
    "// DiagramNode → /webxr/nodes/node/diagram?view=scenarios\n// MCP path: webxr/nodes/node\n// MCP: только пакет, без query и внутреннего пути\n// Выбор UI: variant=Круг\n// Root: подключённый корень; дальше текущие директории и workspace-границы\n// Категории раскрываются до компонента с index.tsx или модуля с src\n// Bound stories retain their authored subject/variant routes\n// packageId remains the exact npm name\ncontext.present(owner)\n// Owner is synchronously attached to its existing Display/HUD projection.\n// Each tile: scale = min(1, availableWidth / ownerWidth, availableHeight / ownerHeight).\n// Viewport and owner resize refit the same production node.\n// Abort or failure releases every child, frame subscription and wrapper.",
  ),
  dependencies: contract(
    "Зависимости из структуры компонента",
    "Нормативные источники — archetypes/README.md#component-dependencies и requirements.md#tabs-routes. Структурный spec даёт ожидаемый состав для представления Зависимости; его вкладка является адресуемым состоянием того же владельца. Эта страница показывает данные и исполнимый пример, не отдельные правила структуры.",
  ...
  typeContract: contract(
    ...
    "Нормативные источники — archetypes/README.md#contract-documentation и archetypes/README.md#documentation-standard. Файлы contract/input.ts и contract/output.ts создают одну вкладку Контракт после Зависимостей; отсутствующий раздел не показывается. README объясняет работу пакета, а TSDoc документирует существующие интерфейсы и функции рядом с кодом.",
    "Пакет @webxr/typedoc самостоятельно разбирает TypeScript 7 и отображает типы, поля, обязательность, значения по умолчанию, описания и примеры. Сигнатуры и типы подсвечивает существующий CodeEditor. Storybook отвечает за обнаружение, адрес владельца и ревизию. Каждый файл input/output экспортирует один основной interface с его TSDoc без документации пакета. Index публикует его через export type вместе с сущностью. Вспомогательные типы находятся в types. Основной тип объявлен в самом контракте; реализация импортирует его оттуда. Импорт своего готового типа из реализации вместо объявления контракта запрещён. Код контракта не исполняется.",
    'component/contract/input.ts → TypeDocDocument → <TypeDoc document={document} />\ncomponent/contract/output.ts → тот же контракт, раздел Выходные данные\nInspector: Вход ↓ / Выход ↑ → Tree полей и вложенных типов\nШапки: Входные поля / Выходные поля; дерево всегда раскрыто, без кнопок сворачивания и разворачивания\nВыбор поля → scrollIntoView в Display; прокрутка Display → собственное видимое описание поля → выделение в полностью раскрытом Tree; показ только строки, без высоты её детей и без смены фокуса\nОтсутствующий input/output не создаёт секцию\nvariant → contract → dependencies → variant: Inspector принадлежит выбранному workspace\nURL: <owner-route>/contract?inspector=input\nСекция Выход: тот же route с inspector=output; Display показывает только выход. По умолчанию — только вход, при его отсутствии — выход',
  ),
  stories: contract(
    "Owner story modules",
    "A catalog stores one static module path and export name for each executable variant. Story modules import production owners, never Storybook." + " Структурные сценарии определены в archetypes/README.md#component-scenarios: spec/scenario.spec.ts или spec/scenario.spec.tsx, включая пустой файл, добавляют вкладку Сценарии по адресу owner?view=scenarios. Без файла вкладки нет. Discovery читает структуру без исполнения; сборка подготавливает варианты поддерживаемой общей фикстуры через readScenario. В Inspector общий Editor показывает JSX с конкретными props, ниже открыт один вариант с его пунктами. В Display один экземпляр компонента меняет props и сохраняет состояние; host центрирует его по реальным границам без изменения размеров.",
  ...
  catalog: contract(
    ...
    "const project = {workspaces: [\"packages/*\", \"!packages/excluded\"]}\n// Пакет доступен без .storybook/manifest.json\n// numeric/index.ts: TSDoc категории, barrel не завершает обход\n// numeric/number/index.tsx: реализация и @packageDocumentation компонента\n// numeric/number/src/helper.ts: внутренний помощник, вынесенный из входа\n// index.tsx содержит один предмет и его композицию; отдельные помощники находятся в src\n// numeric/number/types/: вспомогательные типы предмета\n// shared/: общие исполняемые помощники нескольких компонентов на уровне пакета\n// exports и re-exports не классифицируют категории и компоненты\nconst subject = {\n  id: \"number\",\n  kind: \"component\",\n  label: \"Number\",\n  apiName: \"NumberParameter\",\n  directory: \"numeric/number\",\n  route: \"parameters/number\",\n  presentation: {\n    protocol: \"story-presentation/1\",\n    projection: \"display\",\n    widgets: [\"source\", \"diagnostics\"],\n  },\n  variants: [{\n    id: \"field\",\n    label: \"Field\",\n    route: \"parameters/number/field\",\n    module: {path: \"./stories/number.ts\", export: \"field\"},\n  }],\n}\n// Привязанный number сохраняет одну строку, TSDoc и авторские сценарии\n// Несколько предметов одного модуля остаются под общей строкой\n// src/, shared/, .storybook/, tests/ и test/ скрыты на любой глубине\n// .gitignore исключает dist/; build/ виден, если не игнорируется",
  ),
  workbench: contract(
    "Workbench с одним деревом навигации",
    "`catalog`, `tabs`, `preview`, `inspector` и `status` являются пятью compiled TSX components одного `workbench-layout/3`. Слева одно дерево продолжает путь от репозитория через пакет до его директорий, категорий и предметов; свёрнутые ветви восстанавливаются после перезагрузки. Пакеты и физические директории показывают имена папок; объявленный package.json#label остаётся подсказкой пакета при наведении и участвует в поиске. Виртуальные категории без собственной папки сохраняют авторские названия. Сворачивание родителя закрывает потомков, повторное раскрытие оставляет их свёрнутыми. Обычный выбор строки сохраняет прокрутку и место пакета в общем дереве. Кнопки у поиска находят текущую страницу, раскрывают и сворачивают всё дерево; поиск текущей страницы снимает фильтр и показывает её строку. Весь Workbench монтируется в exact `HUDElement` `external-storybook-workbench`. Status композирует production Breadcrumbs с полным путём `package → nested package → category → subject → variant`; Панель вкладок расположена непосредственно над preview. Нормативный договор Панели вкладок и URL — requirements.md#tabs-routes; эта страница показывает его применение в общем Workbench. Структура проектов и компонентов определяется archetypes/README.md#structure-contract.",
    ...
    "Вкладка задаёт согласованное рабочее пространство просмотра и Inspector по requirements.md#tab-workspace. Контракт получает собственные секции входа и выхода с Tree вложенных типов; зависимости не получают секций предмета. Выбор, поиск и раскрытие восстанавливаются для конкретной вкладки. Содержимое стандартной панели Inspector создаётся при первом выборе и раскрытии: скрытая панель «Исходники» не строит подсвеченную копию примера заранее. После первого раскрытия её nodes сохраняются при переключении и сворачивании, новые values продолжают обновляться. Исполняемая проверка этой последовательности находится в workbench/controller.test.ts. " +
  ...
  server: contract(
    ...
    "Единый процесс владеет HTTP/WebSocket и автоматическим портом для всех подключённых корней и пакетов. Время жизни MCP-подключения независимо. storybook({}) открывает независимый @mcp/root, который сохраняет своё назначение при изменении или отсутствии проектов. @mcp/children формирует единый ответ description, children и path выбранного направления; необязательный label уточняет название. Корень использует тот же Children; подключённый Storybook остаётся обычным проектом разработки. Агент выбирает направление по названию и описанию и передаёт его path следующему вызову. Описания поступают из package.json владельцев через общий каталог. На текущем этапе path заканчивается на пакете; внутренняя документация и параметры не раскрываются. MCP передаёт ответ HTTP-сервера без знания его предметной структуры.",
    ...
    "storybook serve ./workspace\nstorybook_ensure({roots})\nstorybook({}) → {label, description, children}\nstorybook({path: \"webxr\"}) → {path, label, description, children}",
  ),
  browserLifecycle: contract(
    "Current-tab navigation and background agent views",
    "The private browser-lifecycle/ workspace package owns StorybookBrowserLifecycle. Все реальные серверы используют один профиль Chrome пользователя и общий lock запуска. Временный server state хранит записи о вкладках, но не создаёт браузер. Inventory и status читают существующее подключение; Chrome может запустить только явный open. HTTP и isolation-тесты внедряют browser adapter. Вкладка другого origin не присваивается без сохранённой записи владения. Package reservations serialize agent opens: reuse a tab currently showing the exact package, otherwise create a background tab. Multiple tabs may show the same package; listing preserves them all. Readiness uses the full bounded open budget for a busy page. Inventory применяется атомарно после полного наблюдения: abort и неопределённый транспорт сохраняют прежние handles. Scoped status(includeViews:true) и live-check проверяют только выбранный packageId; записи остальных пакетов сохраняются без повторной проверки. Подтверждённо закрытый или сменивший пакет target теряет прежний handle, preflight перед действием сохраняется. Dispatch-aware клиент отмечает createSent у native send boundary после preflight и abort checks; только явный unsent допускает очистку reservation и один retry. Возможная отправка без receipt, старые записи без маркера и ошибки клиента без dispatch-контракта остаются консервативными: blind duplicate create запрещён, прежнее восстановление receipt/наблюдаемого target сохраняется. Indeterminate error возвращает bounded evidence reservation/inventory и read-only attestation до трёх matching targets своего пакета: verified, not-ready, bootstrap-owned либо неопределённость. Query tokens и native IDs скрыты; отсутствие receipt или matching наблюдения не доказывает отсутствие отправки. Диагностика не создаёт targets и не очищает записи. Явный storybook_open с recover: true разрешает повторную попытку только после полного inventory без вкладок пакета; появившаяся исходная вкладка переиспользуется, неопределённая вкладка блокирует сброс. Подтверждённый existing baseline peer можно переиспользовать с обычными readiness/expectedRevision проверками, сохраняя unknown reservation побайтно: peer не считается receipt. Новый не-baseline peer не используется для такого обхода, чтобы navigation не превратила его в ложный late receipt. Без подходящего peer создание остаётся запрещено; настоящий unique late target по исходному URL восстанавливается прежним путём.",
    "Навигация между пакетами, их разделами и общим каталогом сохраняет одну оболочку, Root, Document и Canvas. Контроллер готовит новый контекст, переключает содержимое и URL без полной загрузки; Back/Forward использует тот же путь. Ошибка подготовки или монтирования восстанавливает прежний раздел. Агент открывает вкладки в фоне. Если пользователь перешёл в другой пакет, вкладка сохраняется; прежний viewId не даёт права управлять новым пакетом. При совпадении platform epoch payload использует собственный контроллер пакета из host-сборки, поэтому смена host не блокирует навигацию и сохраняет Root, Document и Canvas. Ошибка перехода доступна через MCP независимо от текущего Inspector. Проверка bridge дожидается завершения смены package scope и заново сверяет ожидаемый пакет. Уход пользователя во время автоматического применения сохраняет готового кандидата без ложной ошибки сборки; монтирование и настоящие ошибки по-прежнему проверяются. Successful watch/subscribe builds enter one bounded automatic-activation queue. The server reuses only an existing exact package page, preserves its route, and requires exact revision/graph/ready/presented/frame/console evidence before updating every tab. A missing or moved page never creates a replacement target. Explicit candidate preview remains unapplied until a later ordinary package subscription or successful live-check.",
  ...
  launcher: contract(
    ...
    [
      ...
      "storybook stop",
      "Переход между пакетами сохраняет Root/Document/Canvas; history и rollback используют один page controller",
    ].join("\n"),
  ),
  scaffold: contract(
  ...
  build: contract(
    ...
    "Каждый пакет сохраняет собственную последовательность ревизий. Общая очередь по умолчанию допускает одну тяжёлую работу и различает queued и compiling; оболочка использует те же slots. Проверенный fingerprint позволяет восстановить неизменённую сборку после перезапуска без compiler worker. Изменение config, ambient types или источников требует новой проверки. Строка состояния показывает реальные этапы каталога, кэша, очереди, отдельных компиляций платформы, оболочки и пакета, ресурсов, проверок, сохранения и применения. Завершение общей операции не скрывает текущую работу пакета. Отмена worker подтверждает исчезновение его собственной группы; кратковременный EPERM повторно проверяется в ограниченный срок, постоянный отказ остаётся ошибкой. Нормативный источник — requirements.md#build-preflight. Build success не заменяет applied revision. Пересозданный scope кандидата сохраняет проверяемую версию при первом сообщении о прежней active revision; применение подтверждается отдельно. Повторный live-check активной версии проверяет текущее состояние и новые ошибки console через тот же протокол без пересборки; прежние ошибки соединения не подменяют его результат. Изменение локальных ссылок README обновляет ресурсы и входы сборки даже при прежнем графе пакетов; удалённый документ не остаётся обязательным входом. Регрессия проверяется в catalog/registry-resources.test.ts.",
    "Агент проверяет candidate через preview и check(live:true); watch/subscribe применяют успешную сборку автоматически. Проверяются точная ревизия, graph, готовый представленный кадр и новые ошибки console. Обычное применение обновляет содержимое существующей страницы, сохраняя URL, browser realm, Root и Canvas. Все вкладки пакета получают обновление; ошибка сохраняет lastWorking. Применённый артефакт переживает перезапуск сервера; после переподключения читатель синхронизирует ревизию. Если рабочей версии нет и сборка не удалась, страница явно показывает это состояние.",
    "status(scope): queue, reason, phase, CPU/RSS, inputFreshness → queued → compiling → built → agent preview → check(live:true) → persisted active/lastWorking\nrestart + verified same inputs → cache hit; no compiler\nCheckWaitTimeout + inProgress → status/wait existing operation",
  ),
  environment: contract(
    ...
    "Browser-only runtime и Workbench не меняют identity резидентного daemon. Shared browser code uses attested source/config inputs and a verified receipt; its worker belongs to the common scheduler. Изменение shared dependencies пересобирает ресурсы на том же сервере. Несовместимые эпохи платформенных modules или host отклоняются без скрытого reload; стили пакета переключаются через semantic links того же Document. Hashed assets remain available to older documents. A failed build preserves the previous working assets and retries after repair. If bootstrap left an owned package page without its bridge, open reloads that same target once after repair. Package pages retain their independent revisions and lastWorking behavior.",
    "registry.readme-updated {nodeIds} → existing Markdown.update\nshared.updated {entry} → explicit restart-required status\npackage.built → candidate only\npackage.updated → all matching package views\npackage.applied-state → recover missed application after reconnect",
  ),
})
