...
const contracts = Object.freeze({
  ...
  workbench: contract(
    ...
    "Catalog, secondary navigation, scenarios, preview, inspector and status are six compiled TSX region components composed by one stable workbench-layout/2 root and projected as a camera-locked overlay. The visually unlabeled scenario button toolbar sits directly above preview; its label is accessibility-only. Expanded primary disclosure groups occupy their header and every visible category row before the next root row.",
    "The external page Experience owns one semantic Document and one Canvas/Renderer/Space host. The host loads the exact @engine/core Inter Regular default font through its stable asset route. Workbench modules separately own controller/state, presentation reparent, navigation model/windowing/rows/tree and the one production Inspector. The status region adapts WorkbenchStatus lead/owner/detail into the exact production StatusBar owner. Primary category, secondary subject and scenario toolbar remain distinct projections. Shared human-facing Inspector chrome and built-in widget titles are Russian; owner values and standard technical identifiers remain exact. Every TSX owner keeps its CSS inside the component.",
