...
const contracts = Object.freeze({
  ...
  catalog: contract(
    ...
    "Главная панель — дерево репозиторий → пакет → вложенный пакет; вторая — категории и предметы выбранного пакета. Предметная панель также показывает обычные директории выбранного репозитория или пакета рядом с JSON-разделами: имя берётся с диска, вложенность сохраняется, выбор открывает README.md. src, .git, node_modules, .storybook, tests и test скрыты на любой глубине; остальные исключения задаются Git через git check-ignore --no-index, включая вложенные .gitignore и правила с !. Директории с package.json сохраняют отдельное владение и не обходятся как обычные папки. Вложенность не объединяет сборки и ревизии. Выбор пакета открывает содержимое в текущей вкладке по /packages; старый /browse перенаправляет туда. Сборки и ревизии остаются изолированными по packageId. Структура определяет состав пакетов через package.json#workspaces и Bun.Glob, включая звёздочки и исключения. Пакет без манифеста тоже видим и выбираем; его страница без исполняемых модулей использует только общие compiler owners Storybook. Манифест дополняет пакет содержимым; manifest.packages остаётся только для проектов без workspaces. JSON reader в discovery/declarations.ts передаёт нормализованный StorybookCatalog из catalog/catalog.t.ts. ExternalStorybookRegistry принимает StorybookCatalogResolver при создании; граф и подготовка сборки не импортируют JSON reader. Структура и манифест дают один граф. Состав структурного проекта больше не дублируется в манифесте. " +
