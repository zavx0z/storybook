...
- Шрифт страницы загружается через exact `@zavx0z/engine/default-font` и asset
  ...
  запрещены.
- Репозиторий владеет собственной документацией из README, TSDoc, contract и spec.
...
- Собственная документация пакета `@zavx0z/storybook` обнаруживается по его
  `package.json`, README, TSDoc, contract и spec. Она подключается к тому же
...
- README, TSDoc, контракты и сценарии должны оставаться согласованными с
  действующим кодом. Изменение общей реализации обновляет описание владельца.
