...
плоскости Display. Его физические атрибуты `width=600`, `height=337.5` в мм,
CSS-разрешение `2268 × 1276 px` и `scale: 1` постоянны; `dpi` вычисляется.
Host вписывает поверхность в обе оси свободного прямоугольника HUD
дистанцией и параллельным сдвигом ViewPoint вместе с целью. При несовпадении
пропорций остаются поля. Resize не изменяет поверхность, её матрицу и layout.
