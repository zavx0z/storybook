...
Пакеты и физические директории показаны по именам папок; `label` пакета виден
в подсказке при наведении. Поиск принимает оба имени пакета.
Сворачивание родителя оставляет потомков свёрнутыми и при повторном раскрытии.
Кнопки у поиска находят текущую страницу, раскрывают или сворачивают всё дерево;
поиск текущей страницы снимает фильтр и показывает её строку без смены фокуса.
Обычный выбор строки сохраняет прокрутку и положение пакета в общей иерархии.
