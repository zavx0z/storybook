...
const packageScenarioPath = resolve(import.meta.dir, "../../../archetypes/package/spec/scenario.spec.ts")
...
test("полная история имеет читаемый snapshot", () => {
  ...
  const localPath = (path: string) => path.replace(sourceRoot, "<storybook>")
  expect({
