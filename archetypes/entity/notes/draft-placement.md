...
Собственного `index.ts` достаточно для обнаружения этих контрактов и сценариев;
наличие внутренних помощников не определяет доступность публичного описания.
...
модульный TSDoc. Для корня пакета действует тот же выбор исходника и общий
читатель модульного TSDoc. README.md не является источником обзора;
...
Служебные `src`, `shared`, `contract`, `spec`, `fixture`, test-директории и директории с
