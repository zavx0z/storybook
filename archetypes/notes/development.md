# Когда выделять категорию или пакет

```mermaid
flowchart TD
  change["Появились новые части"] --> internal["Служат одной сущности"]
  internal --> src["Остаются внутренней реализацией"]
  change --> peers["Равноправные сущности с общим признаком"]
  peers --> category["Категория"]
  change --> whole["Несколько самостоятельных экспортов одного целого"]
  whole --> package["Пакет"]
```
