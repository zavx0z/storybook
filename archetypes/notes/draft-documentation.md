...
Ниже сохранены прежние подробные правила оформления; использование README пока остаётся в силе.

```mermaid
flowchart LR
  meaning["Общий смысл"] --> owner["Заметка общего владельца"]
  behavior["Поведение функции"] --> code["TSDoc объявления"]
  data["Вход и выход"] --> contract["Контракт"]
  evidence["Ожидаемое поведение"] --> tests["Спецификации"]
```
