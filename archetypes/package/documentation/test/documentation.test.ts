...
test("читает только модульный TSDoc, сохраняя Markdown и примеры", () => {
  ...
  const doc = readModuleDocumentation({source, path: "/owner/index.ts"})!
...
test("не подменяет документацию комментариями сущностей или строками", () => {
  for (const source of [
    '/** Описание класса */\nexport class Sample {}',
    'export const text = "/** @packageDocumentation */"',
    'import "./side-effects"\n/**\nНе обзор\n@packageDocumentation\n*/',
    '/**\n```ts\n@packageDocumentation\n```\n*/',
  ]) expect(readModuleDocumentation({source, path: "index.ts"})).toBeNull()
  expect(() => readModuleDocumentation({source: '/** One\n@packageDocumentation\n*/\n/** Two\n@packageDocumentation\n*/', path: "index.ts"})).toThrow("Multiple")
})
