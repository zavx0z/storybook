...
Ревизия пакета раскрывает модульный TSDoc и исполняемые источники владельца.
README остаётся указателем на заметки и не входит в содержимое ревизии.
...
[readPackageJson](../index.ts) сохраняет необязательный `label`, а
[сценарий пакета](../../spec/scenario.spec.ts) проверяет его содержание только
при наличии. [Обнаружение пакетов](../../../../discovery/packages.ts) использует
имя пакета, когда подписи нет. Локальное чтение само по себе не подтверждает
все правила обнаружения и независимого обновления пакетов.
