...
Здесь описан выбор репозиториев, которые показывает Storybook: как путь добавляется
в общий список, что происходит при удалении и как сохраняется выбор.
Это описание прежней реализации; детали интерфейса и хранения требуют сверки.

```mermaid
flowchart LR
  choose["Выбор директории"] --> confirm["Подтверждение точного пути"]
  confirm --> registry["Общий список репозиториев"]
  registry --> ui["Каталог в интерфейсе и MCP"]
  remove["Удаление из списка"] --> registry
```
...
## Общий список для интерфейса и MCP
