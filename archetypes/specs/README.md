...
Проверяемая функция вызывается непосредственно в callback `describe.each`
перед тестами; тесты проверяют общий результат своего варианта. Декларация
`runtime` для запуска не используется. Подробное правило и пример принадлежат
[документации сценариев](./scenarios/README.md#подготовка-данных-и-тело-теста).
