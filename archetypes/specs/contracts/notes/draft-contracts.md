...
Обзор пакета должен быть выражен в коде и публичных контрактах; текущий
`readPackage` всё ещё читает README. Документация входного модуля — его index.
...
единственный отображаемый документ; отсутствующий раздел не показывается.
Контракт принадлежит физической директории своего модуля.
...
Вкладка сохраняет физический pathname владельца и выбирается
`?view=contract` по [Route](../../../../route/README.md) и
[контракту вкладок](../../../../requirements.md#tabs-routes). Прямое открытие,
перезагрузка и history согласованы с этим UI-адресом. MCP пока адресует только
пакет целиком по [своему контракту](../../../../mcp/address/README.md).
