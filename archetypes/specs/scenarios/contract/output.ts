...
/**
Результаты применения правил оформления и выполнения сценария.

@property status - Общий итог: `passed`, `failed` или `incomplete`.
Незавершённые проверки не считаются пройденными.

@property checks - Результат каждого правила: идентификатор `rule`, статус и список
проблем с пояснением и положением в исходнике при наличии.
`not-checked` обозначает правило, которое не было проверено.
*/
...
@property validation - Результат проверки правил {@link ScenarioValidation};
