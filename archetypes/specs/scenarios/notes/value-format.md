...
Собственные индексные элементы массива также читаются через дескрипторы:
getter и setter сохраняются меткой `{$type: "accessor", get, set}` без выполнения
и без замены исходного свойства. Пользовательские `map` и итератор массива
для обхода не вызываются. Неперечисляемый индекс остаётся элементом массива.
