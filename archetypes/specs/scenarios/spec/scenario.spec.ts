...
describe.each([
  ...
  describe("Результат и проверяемые условия", () => {
    test("Получение результата", () => {
      expect(
        variant?.setup,
        "Проверяемая функция или компонент вызывается прямо внутри describe перед тестами, без декларации runtime. Результат этого варианта становится общим основанием его проверок; асинхронное выполнение завершается через await.",
      ).toMatch(/\S/u)
    })
    test.todo("actual", () => {
      expect(
    ...
    test("Состав объекта", () => {
      expect(
        result.checks.find(check => check.matcher === "toEqual")?.source,
        "Полный состав объекта раскрывается через toEqual с явно заданными полями. Ожидание не собирается из Object.keys(actual); затем существенные поля разбираются отдельными пунктами.",
      ).toMatch(/\S/u)
    })
  ...
  describe("Подготовка и жизненный цикл", () => {
    ...
    /** @remarks Пример без выделяемых ресурсов может не использовать hooks. */
    test.skipIf(result.hooks.length === 0)("Освобождение ресурсов", () => {
      expect(
        result.hooks.join("\n"),
        "Штатные hooks Bun Test связывают создание и освобождение ресурсов с их жизненным циклом. Общие ресурсы варианта освобождаются после него; ресурсы отдельного теста подготавливаются и очищаются отдельно.",
      ).toMatch(/\S/u)
    })
  })
