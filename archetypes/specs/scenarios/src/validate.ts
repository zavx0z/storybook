...
import type {ScenarioExecution, ScenarioSource, ScenarioValidation} from "./types"

/** Применяет реализованные правила архетипа к структуре и одному завершённому запуску. */
export function validateScenario(source: ScenarioSource, execution: ScenarioExecution): ScenarioValidation {
  const checks: ScenarioValidation["checks"][number][] = []
  const location = {path: source.path, line: 1, column: 1}
  const add = (rule: string, issues: ScenarioValidation["checks"][number]["issues"]) => {
