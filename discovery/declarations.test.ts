...
import {createExternalStorybookGraph, externalStorybookNode} from "../catalog/graph.ts"
...
describe("external Storybook JSON declarations", () => {
  ...
  test("reads project and package labels from package.json on every resolution", async () => {
    ...
    await updateJson(componentsPackageJson(root), value => ({...value, label: "Компоненты проекта", description: "Элементы интерфейса проекта"}))
    ...
    expect(before.description).toBe("Элементы интерфейса проекта")
    await updateJson(componentsPackageJson(root), value => ({...value, label: "Новое название", description: "Обновлённое назначение"}))
    ...
    expect(after.description).toBe("Обновлённое назначение")
    expect(after.canonicalId).toBe(before.canonicalId)
    expect(after.digest).not.toBe(before.digest)
    await updateJson(componentsPackageJson(root), value => ({...value, description: "Назначение без смены имени"}))
    const refreshed = declarationPackage((await resolveExternalStorybookDeclarations([root])).scopes, "@fixture/components")
    expect(refreshed.description).toBe("Назначение без смены имени")
    expect(refreshed.label).toBe(after.label)
    expect(refreshed.digest).not.toBe(after.digest)
  ...
  test("resolves a workspace and independent package through canonical owner paths", async () => {
    ...
    const components = declarationPackage(resolved.scopes, "@fixture/components")
  ...
  test("preserves paired semantic category identity and rejects partial metadata", async () => {
    ...
    const resolved = declarationPackage(
      (await resolveExternalStorybookDeclarations([typed])).scopes,
      "@fixture/components",
    )
  ...
  test("preserves the declared owner path independently of a hardlink leaf alias", async () => {
    ...
    const components = declarationPackage(declarations.scopes, "@fixture/components")
  ...
  test("resolves only exact ordered public CSS exports and digests their bytes", async () => {
    ...
    const first = declarationPackage(
      (await resolveExternalStorybookDeclarations([digestRoot])).scopes,
      "@fixture/components",
    )
    await Bun.write(
    ...
    const consumer = declarationPackage(
      (await resolveExternalStorybookDeclarations([dependencyResource])).scopes,
      "@fixture/docs",
    )
    ...
    const transitiveConsumer = declarationPackage(
      (await resolveExternalStorybookDeclarations([transitiveResource])).scopes,
      "@fixture/docs",
    )
  ...
  test("validates the exact widget registry and subject-owned presentation", async () => {
    ...
    const selfPackage = declarationPackage(
      (await resolveExternalStorybookDeclarations([self])).scopes,
      "@zavx0z/storybook",
    )
...
function declarationPackage(
  ...
  return declaration as StorybookPackage
}
