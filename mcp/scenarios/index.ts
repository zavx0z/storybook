...
import type {ScenariosInput} from "./contract/input"
import type {ScenariosOutput} from "./contract/output"

export type {ScenariosInput, ScenariosOutput}
...
export function presentScenarios(input: ScenariosInput): ScenariosOutput {
  ...
  if (!input.prepared) return {owner, source, status: source === null ? "absent" : "pending", revision: null, variants: [], items: []}
  ...
  return {owner, source, status: "ready", revision, variants, items}
}
