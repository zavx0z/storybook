...
@param statusFromResult - Управляющие инструменты используют lifecycle status;
у HTTP-прокси передаётся false, поскольку status внутри его JSON является предметными данными.
*/
export async function traceMcpRequest<T>(
  ...
  try {
    ...
    await record({...entry, durationMs: Date.now() - startedAt, status: statusFromResult && ["failed", "timeout", "unavailable"].includes(String(status)) ? "failed" : "success", ...(captureId === undefined ? {} : {captureId}), result: JSON.stringify(sanitizeMcpValue(report), null, 2) ?? "null"})
    return result
  } catch (error) {
