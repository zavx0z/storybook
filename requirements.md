...
с `package.json`. Содержимое определяется его авторами и здесь не нормируется.
...
в `index.tsx`. Если выносить нечего, `src/` не нужна; она предназначена только
для крупных частных помощников и вычислений. Код нескольких компонентов
размещается в `shared/` на уровне пакета. Пустую `src/` ради обнаружения не создают.
