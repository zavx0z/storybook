...
Физические атрибуты Display задают `600 × 337,5 мм`, CSS-разрешение —
`2268 × 1276 px`, масштаб — `1`; `dpi` вычисляется по обеим осям.
Размеры окна и свободной области HUD управляют только
дистанцией и параллельным сдвигом ViewPoint с целью. Поверхность вписывается
целиком с сохранением пропорций и полями. Resize сохраняет CSS viewport,
матрицу `2268 × 1276`, раскладку содержимого, прокрутку, фокус и DOM identity.
