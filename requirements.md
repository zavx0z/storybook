...
Обычная директория использует исключительно модульный TSDoc `index.ts`.
...
Опциональный `directory` у subject задаёт package-relative путь существующего
обнаруженного модуля с `src/`. Это связь с тем же структурным узлом, а не
объявление нового владельца, API export или маршрута. Отсутствующий путь,
категория без `src/` и пересечение границы пакета отклоняются.
...
корневой пакет → вложенные пакеты. Родитель — ближайший пакет в цепочке
содержащих каталогов. Корневой пакет представлен одним выбираемым узлом
без оболочки project/repository. Это общее правило, включая Storybook.
