...
Это единственный нормативный источник правил структуры для авторов и агентов:
выбор проектов, идентичность и состав пакетов, каталоги, компоненты и их документация.
README, ARCHITECTURE, archetypes и self-docs дают навигацию и примеры, а не отдельные
версии этих норм. Перед изменением структуры агент читает этот раздел.
Маршруты Dependencies и вкладок определены в [контракте Панели вкладок](#tabs-routes),
...
Общие законы остаются в нормативном документе общего владельца; дочерние README
...
Index также явно реэкспортирует через `export type` основные интерфейсы своего
input/output из contract вместе с сущностью. Это разрешённая публикация контрактов;
вспомогательные типы и helpers через index не экспортируются. Одна runtime-сущность
и её контрактные type-only exports не противоречат друг другу.
Правило относится к входу предмета, а не к пакетному агрегатору публичного API.
...
Служебная types без самостоятельной границы модуля также скрыта. Существующий
модуль с именем types и собственной границей index.tsx или src сохраняется:
имя служебного каталога не отменяет уже поддержанную структуру модуля.
...
Правила привязки `subject.directory` определены в [контракте каталогов и компонентов](#component-placement); presentation и widgets остаются ответственностью этого раздела.
...
Authored variant URL сохраняются. Для найденного dependency spec выводится
отдельная вкладка по <owner-route>/dependencies: например diagram/dependencies
или dir-category/dir-module/dependencies. Это route state того же владельца,
а не новый узел, категория или запись manifest. Коллизия с authored route
отклоняется при построении графа; одноимённая физическая папка имеет dir-dependencies.
Допускается только обнаруженная цепочка директорий и её конечный dependencies;
произвольные mixed пути не принимаются. Таблица маршрутов, immutable revision,
...
Главная и предметная панели отображают иерархию из [единого контракта структуры](#structure-contract).
...
Правила состава дерева и границ модулей заданы в [едином нормативном разделе](#structure-contract).
...
Его URL и возвращение к обзору определены в [контракте Панели вкладок](#tabs-routes);
...
Она содержит входные и выходные данные в порядке input, output; отсутствующий
раздел не показывается. Контракт наследуется при привязке subject.directory.
