...
[правилу отображения контракта](#contract-documentation). Параметры ноды,
...
Совпадение эпох платформенных модулей и кода host проверяется до замены
содержимого. Изменение этих эпох требует явно обозначенного перезапуска
страницы; автоприменение не делает скрытый reload и
не объявляет корректно собранный несовместимый кандидат ошибкой компиляции.
...
Раскрытие дерева сохраняется для своей вкладки. Идентификаторы учитывают
...
«Входные поля» и «Выходные поля» сохраняются. Справа находятся
кнопки «Свернуть всё» и «Развернуть всё» с готовыми
значками общего UI, как в дереве Интерпретатора. Они управляют всеми
ветвями только своего направления. Выбор строки оглавления переходит к
...
автоматически определяет фактически видимое описание, раскрывает его родителей
и выделяет строку дерева, не перехватывая фокус. Последний клик не подменяет
