// Skill-local regression for data-only background interactions.
...
describe("Storybook data-only interaction plans", () => {
  ...
  test("requires an existing exact package route and target before interaction", () => {
    const valid = {
      packageName: "@ui/storybook",
      route: "/integer-input/default",
      targetId: "target-1",
      targetUrl: "http://storybook.test/integer-input/default",
      currentUrl: "http://storybook.test/integer-input/default",
    }
    expect(() => validateInteractionInvocation(valid)).not.toThrow()
    expect(() => validateInteractionInvocation({...valid, packageName: ""})).toThrow("exact package")
    ...
    expect(() => validateInteractionInvocation({...valid, currentUrl: "http://storybook.test/color-input/default"})).toThrow("exact route")
  })

  test("accepts only preserved-route, console-clean and written canvas evidence", () => {
    const accepted = {
      targetUrl: "http://storybook.test/integer-input/basic/labeled",
      initialUrl: "http://storybook.test/integer-input/basic/labeled",
      finalUrl: "http://storybook.test/integer-input/basic/labeled",
      console: [{level: "log"}],
      captures: [{kind: "exact-canvas-png", written: true}],
    }
    expect(() => assertInteractionEvidence(accepted)).not.toThrow()
    expect(() => assertInteractionEvidence({...accepted, finalUrl: "http://storybook.test/color-input/basic"})).toThrow("exact route")
