...
/** Data-only background interaction plan for one exact storybook route. */
...
/** Require an already loaded exact package route and target before dispatch. */
export function validateInteractionInvocation(invocation: InteractionInvocation): void {
  if (!/^@[a-z0-9][a-z0-9._-]*\/storybook$/u.test(invocation.packageName)) {
    throw new Error("interact requires an exact package name")
  }
