...
describe("shared Storybook no-HMR server", () => {
  ...
  test("serves only canonical registered routes with a structured Russian shell", async () => {
    ...
    try {
      ...
      expect(rootHtml).toContain(".storybook-footer {\n        position: static")
      ...
      expect(html).toContain("background: var(--storybook-shell-background)")
      expect(html.indexOf('rel="stylesheet"')).toBeLessThan(html.indexOf("--storybook-shell-background"))
      expect(html).toContain('<meta name="storybook-status-bar-lead" content="Создано для">')
      expect(html).toContain('<meta name="storybook-status-bar-owner" content="MetaFor">')
      expect(html).toContain('<meta name="storybook-status-bar-detail" content="переиспользуемая WebGPU-инфраструктура UI">')
