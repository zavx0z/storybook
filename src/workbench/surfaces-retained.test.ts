...
const items: readonly StorybookNavigationItem<Route>[] = [
...
const snapshots = (
  surface: {node: Object3D; diagnostics: StorybookRetainedDiagnostics},
): ReadonlyMap<string, OwnerSnapshot> => new Map(
  surface.diagnostics.owners.map(({key}) => [key, snapshot(owner(surface, key))] as const),
)

const materializations = (diagnostics: StorybookRetainedDiagnostics): Readonly<Record<string, number>> => Object.fromEntries(
...
describe("retained @zavx0z/storybook Workbench surfaces", () => {
  ...
  test("keeps Navigation owners stable and rematerializes only active or disabled items", () => {
    ...
    const reconciledItems: readonly StorybookNavigationItem<Route>[] = [
      disabledItems[2]!,
      disabledItems[1]!,
    ...
    expect(() => surface.setOptions({
      ...
      route: "second",
      onNavigate: navigate,
    })).toThrow("Duplicate storybook navigation item id")
    expect(surface.diagnostics.materializations).toBe(14)
    surface.dispose()
  })

  test("filters and bounds a large grouped navigation without materializing the whole catalog", () => {
    const largeItems: readonly StorybookNavigationItem<Route>[] = Array.from({length: 1000}, (_, index) => ({
      id: `item-${index}`,
    ...
    const view = selectStorybookNavigationItems({
      title: "Каталог",
      items: largeItems,
      route: "first",
      query: "needle",
    ...
    const searchSurface = new StorybookNavigationSurface<Route>({
      title: "Каталог",
      items: largeItems,
      route: "first",
      query: "needle",
    ...
    const surface = new StorybookNavigationSurface<Route>({
      title: "Каталог",
      items: expandedItems,
      route: "first",
  ...
  test("keeps Dock owners and counters stable across a pure retained-root transform", () => {
    const navigate = (): void => {}
    const surface = new StorybookDockSurface<Route>({title: "Routes", items, route: "first", onNavigate: navigate})
    surface.attachCanvas(createFakeRuntime())
  ...
  test("keeps one enabled keyboard focus, activates the live route callback, and shares it with pointer clicks", async () => {
    ...
    const disabledItems: readonly StorybookNavigationItem<Route>[] = [
      items[0]!,
  ...
  test("keeps accordion section disclosure independent from leaf route and keyboard focus", () => {
    const toggles: Array<readonly [string, boolean]> = []
    const routes: Route[] = []
    const grouped: readonly StorybookNavigationItem<Route>[] = [
    ...
    const options = (items: readonly StorybookNavigationItem<Route>[]) => ({
      title: "Catalog",
      items,
    ...
    const surface = new StorybookNavigationSurface<Route>(options(grouped))
    surface.attachCanvas(createFakeRuntime())
    surface.setRect({x: 0, y: 0, w: 210, h: 640}, 0.001, font)
    surface.onActivate()
  ...
  test("applies the same enabled keyboard order to Dock", () => {
    const calls: Route[] = []
    const dockItems: readonly StorybookNavigationItem<Route>[] = [
      items[0]!,
      {...items[1]!, disabled: true},
      items[2]!,
    ]
    const surface = new StorybookDockSurface<Route>({
      title: "Routes",
      items: dockItems,
      route: "first",
  ...
  test("shows every control kind but only invokes boolean and select controls", () => {
    ...
    const surface = new StorybookStoryPanelSurface({
      ...
      controls: [
        ...
        {key: "label", label: "Текст", group: "Значения", kind: "text", interactive: false},
        {key: "color", label: "Цвет", group: "Значения", kind: "color", interactive: false},
    ...
    const ownerKeys = surface.diagnostics.owners.map(({key}) => key)
    for (const key of [
      "source-control:count",
      "source-control:label",
      "source-control:color",
      "source-control:custom",
  ...
  test("reconciles Info descriptors while status dirties only its exact owner and dispose cleans all parents", () => {
    const surface = new StorybookInfoSurface({
      title: "Contract",
    ...
    expect(() => surface.setOptions({
      ...
      status: "second",
    })).toThrow("Duplicate storybook info line id")
    expect(surface.diagnostics.materializations).toBe(9)
